...
SELECT 
    ...
    SUM(CASE WHEN imagen LIKE '%consolas/%' OR imagen LIKE '%perifericos/%' OR imagen LIKE '%polerones/%' OR imagen LIKE '%entretenimiento/%' THEN 1 ELSE 0 END) as rutas_antiguas,
    SUM(CASE WHEN imagen LIKE '%data:image%' THEN 1 ELSE 0 END) as base64_images,
    SUM(CASE WHEN imagen LIKE '%play4.png%' OR imagen LIKE '%play5white.png%' OR imagen LIKE '%mandoplay.png%' OR imagen LIKE 'https://levelup-gamer-products.s3.us-east-1.amazonaws.com/img/%' THEN 1 ELSE 0 END) as rutas_nuevas
FROM productos;
